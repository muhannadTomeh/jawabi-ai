...
export function UsersList() {
    ...
    <Card>
      ...
      <CardContent>
        {users.length === 0 ? (<p className="text-center text-muted-foreground py-8">
            ...
            <TableBody>
              {users.map((user) => (<TableRow key={user.id}>
                  ...
                  <TableCell>
                    {user.role === 'admin' ? (<Badge className="bg-amber-500/10 text-amber-600 hover:bg-amber-500/20">
                        <Shield className="h-3 w-3 ml-1"/>
                        أدمن
                      </Badge>) : (<Badge variant="secondary">مستخدم</Badge>)}
                  </TableCell>
                  <TableCell>
